...
- **支持分割大型日志文件**：对于大于 800MB 的 .log 文件，可以将其分割为多个较小的文件
...
对于大于 800MB 的 .log 文件，可以使用分割功能将其拆分为多个较小的文件：

1. 在资源管理器中右键点击大于 800MB 的 .log 文件
2. 选择 "Split File" 选项
3. 输入分割后单文件大小（MB），直接确认则使用默认值 500MB
4. 等待分割完成，分割后的文件将保存在文件当前目录下新创建的同名子目录中
5. 分割后的文件命名为 `xxx_index.log`，其中 `xxx` 为原文件名，`index` 为分割后的文件顺序序号（从 1 开始）
6. 分割完成后，源文件将被自动删除
